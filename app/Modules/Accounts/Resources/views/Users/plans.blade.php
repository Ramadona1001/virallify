    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('assign_user_plans') }}" method="post">
                        @csrf
                        <input type="hidden" name="user_id" value="{{ $user->id }}">
                        <label for="plan_id">{{ transWord('Select Plan') }}</label>
                        ...
                        <div class="col-12">
                            <div class="table-responsive pt-0">
                                <table id="example" class="table table-bordered dt-responsive nowrap"
                                    ...
                                    <tbody>
                                    @foreach($user_plans as $index => $p)
                                        <tr  class="row_{{$p->plan->id}}">
                                            <td>{{$index + 1}}</td>

                                            <td>
                                              {{$p->plan->translate()->name}}
                                            </td>

                                            <td>
                                                {{$p->plan->translate()->content}}
                                            </td>

                                            <td>{{ $p->plan->wash_number }}</td>
                                            ...
                                            <td>
                                                <ul style="max-width: 50px" class="dtr-details" >
                                                    <li>
                                                        <a href="{{ route('remove_users_plans',['user_plan'=>$p]) }}" class="btn btn-danger btn-sm" onclick="return confirm('Are You Sure?')">
