...
@section('content')
            ...
            <div class="card">
                <div class="card-header bg-white d-flex align-items-center justify-content-between border-bottom">
                    ...
                    <div style="text-align: end" class="add_btn">
                        <a id="" class="btn btn-primary" href="javascript:void(0);" data-toggle="modal" data-target="#home_sectionsModal">
                        ...
                        <div class="col-12">
                            <div class="table-responsive pt-0">
                                <table id="example" class="table table-bordered dt-responsive nowrap"
                                    ...
                                    <tbody>
                                    @foreach($home_sections as $index => $home_section)
                                        <tr  class="row_{{$home_section->id}}">
                                            <td>{{$index + 1}}</td>
                                            <td>
                                              {{$home_section->translate()->name}}
                                            </td>
                                            
                                            <td>
                                              {{$home_section->translate()->sub_title}}
                                            </td>

                                            <td>
                                                {{$home_section->translate()->content}}
                                            </td>

                                            <td>
                                                <a href="{{ $home_section->btn_url }}" class="btn btn-primary">{{ $home_section->btn_text }}</a>
                                            </td>

                                            <td>
                                                {{ $home_section->order_no }}
                                            </td>
                                            ...
                                            <td>
                                                <ul style="max-width: 50px" class="dtr-details" >
                                                    <li>
                                                        <a href="{{route('edit_home_sections',['home_section' => $home_section])}}"
                                                           class="text-info"
                                                           title="Edit"
                                                    ...
                                                    <li>
                                                        <a onclick="return confirm('{{ transWord('Are You Sure?') }}')"  href="{{route('delete_home_sections',['home_section' => $home_section] )}}" class="text-danger sa-delete"  title="Delete" data-original-title="Delete">
    ...
    <div class="modal fade" id="home_sectionsModal" tabindex="-1" role="dialog"
         aria-labelledby="exampleModalScrollableTitle" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable modal-lg">
            <div class="modal-content ">
                <div class="modal-header">
                    <h5 class="modal-title mt-0" id="exampleModalScrollableTitle">{{ transWord('New Home Section') }}</h5>
                ...
                <div class="modal-body p-3">
                    <form action="{{route('store_home_sections')}}" method="post" enctype="multipart/form-data">
                        @csrf
