            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @if ($partner->image != null)
                        <img src="{{ asset($partner->image) }}" style="display:block;margin-left:auto;margin-right:auto;width:200px;" class="img-thumbnail">
                        <hr>
                    @else
                        {{ transWord('No Image') }}
                    @endif
                    ...
                    <div class="row">
                        <div class="col-12">
                            <form action="{{route('update_partners' , ['partner' => $partner])}}" method="post" enctype="multipart/form-data">
                                @csrf
                                        ...
                                        @foreach (config('app.languages') as $key => $lang)
                                            <div class="tab-pane fade  @if ($loop->index == 0) show active @endif" id="nav-{{ $key }}" role="tabpanel" aria-labelledby="nav-{{ $key }}-tab">
                                                <div class="form-group mt-3 col-md-12">
                                                    ...
                                                    <input value="{{$partner->translate($key)->name}}" id="name" type="text" name="{{ $key }}[name]" class="form-control" placeholder="{{ transWord('Name') }}" required>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
